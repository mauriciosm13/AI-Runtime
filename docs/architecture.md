...
`ProviderExecutor` in `application/resilience/` applies bounded retries and static cross-provider failover. It classifies `ProviderError.retryable`, skips routes blocked by entitlements or missing adapters, and returns the provider that actually served the response for usage accounting.
