...
`POST /v1/responses` requires bearer API-key authentication before generation. The API layer extracts `Authorization: Bearer airt_...`, invokes `AuthenticateApiKey`, and injects an `AuthenticatedPrincipal` (no plaintext secret or hash). Missing or invalid credentials map to `401 Unauthorized`; suspended organizations map to `403 Forbidden`. After auth, the route validates a JSON body with Pydantic schemas, maps it to `CreateResponseCommand` (generation request plus `request_id`, `organization_id`, and `api_key_id`), and either invokes `CreateResponse.execute` for JSON or `CreateResponse.stream` for SSE when `stream` is true. Provider failures map to `502 Bad Gateway`; validation failures map to `422 Unprocessable Entity`. Errors use the standardized envelope in `api/schemas/errors.py` via `api/exception_handlers.py`. The composition root wires `ModelRouter` (OpenAI, Anthropic, and Gemini adapters when configured), SQLAlchemy repositories (including usage), `StaticCostEstimator`, and `Argon2ApiKeyHasher` through FastAPI dependencies and stores a shared `httpx.AsyncClient` on the application lifespan. Unknown catalog models map to `400 Bad Request` (`unsupported_model`); catalog models whose provider adapter is not registered map to `503 Service Unavailable` (`provider_error`); organization entitlement denials map to `403 Forbidden` (`model_not_available`).
...
`CreateResponse` in `application/responses/` receives a `CreateResponseCommand`, delegates generation to `ProviderExecutor` (which resolves the model through `ModelRouter`, retries transient provider failures, and optionally fails over to alternate catalog models), estimates cost through `CostEstimator`, persists a `UsageRecord` through `UsageRepository` after a successful provider response, and returns the `GenerationResponse`. The streaming path (`CreateResponse.stream`) applies the same rate-limit and organization-policy gates, rejects `Idempotency-Key`, yields provider-neutral deltas, and persists usage only after a completed stream event. Prompt/response content is not stored. Both paths are exposed through `POST /v1/responses` in the API layer.

`ResolvePrompt`, `CreatePromptVersion`, and `GetPromptVersions` in `application/prompts/` load and render organization-scoped prompt templates through the `PromptRepository` port; `BuildContext` in `application/context/` enforces an opt-in input-token budget using the `TokenCounter` port and the static `DEFAULT_MODEL_CONTEXT_WINDOWS` catalog. The API layer resolves and budgets messages before calling `CreateResponse`, which is unchanged. Templates are stored as configuration (`prompt_templates`); variable values and rendered content are never persisted. See ADR 0008.

`ProviderExecutor` in `application/resilience/` applies bounded retries and static cross-provider failover. It classifies `ProviderError.retryable`, skips routes blocked by entitlements or missing adapters, and returns the provider that actually served the response for usage accounting. `ProviderExecutor.stream` uses the same route chain; retries and failover stop once the first stream event has been yielded.

`ModelRouter` in `application/routing/` binds the domain catalog (`model → provider`) to registered `ModelProvider` adapters. It does not call providers, enforce organization policy, or persist usage. The initial catalog is `DEFAULT_MODEL_CATALOG` in `domain/routing.py`; optional failover targets live in `DEFAULT_FAILOVER_CATALOG`.
...
`ModelProvider` in `ports/model_provider.py` is an asynchronous contract with `generate` (one-shot `GenerationResponse`) and `stream` (async iterator of `GenerationDelta` then a terminal `GenerationResponse`). Concrete adapters under `providers/` implement this interface.
...
The first concrete adapter is `OpenAIModelProvider` under `providers/openai/`. It implements `ModelProvider` by mapping `GenerationRequest` / `GenerationResponse` to the OpenAI Chat Completions HTTP API using `httpx`, including `stream=true` SSE for `stream()`. It does not decide routing, authorization, retries, failover, or tool calling. The composition root registers it with `ModelRouter` under the name `"openai"`.

`AnthropicModelProvider` under `providers/anthropic/` implements the same port against Anthropic Messages (`POST /v1/messages`) using `httpx` with `x-api-key` and a pinned `anthropic-version` header. System messages from `GenerationRequest.messages` map to Anthropic's `system` field; remaining messages stay in `messages`. `stream()` sends `stream=true` and maps `content_block_delta` events to `GenerationDelta`. When `AI_RUNTIME_ANTHROPIC_API_KEY` is blank, the composition root skips registering the adapter so OpenAI-only deploys stay valid. A catalog model routed to `"anthropic"` without a registered adapter fails with `ProviderNotRegisteredError` (HTTP `503`, `provider_error`).

`GeminiModelProvider` under `providers/gemini/` implements the same port against Gemini `generateContent` (`POST /v1beta/models/{model}:generateContent`) using `httpx` with an `x-goog-api-key` header. System messages map to Gemini's `systemInstruction`; assistant messages map to contents with role `model`. `stream()` calls `streamGenerateContent?alt=sse`. When `AI_RUNTIME_GEMINI_API_KEY` is blank, the composition root skips registering the adapter. A catalog model routed to `"gemini"` without a registered adapter fails with `ProviderNotRegisteredError` (HTTP `503`, `provider_error`).
...
The infrastructure package `infrastructure/redis/` constructs the shared async Redis client (`create_redis_client`) and adapters for organization token-bucket rate limiting (`RedisRateLimiter`), Idempotency-Key coordination (`RedisIdempotencyStore`), and opt-in response cache (`RedisResponseCache`). The API lifespan stores the Redis client on `app.state.redis`. Rate limiting, idempotency, and response cache fail open when Redis is unavailable.
...
The first telemetry capability is structured JSON request logging in `telemetry/logging.py`, configured during application bootstrap. Request lifecycle logs are emitted by API middleware and include `request_id`, `trace_id` (the same value), `span`, HTTP method, path, status code, and duration. Logs intentionally exclude secrets, authorization headers, and message bodies. In-process Prometheus metrics live in `telemetry/metrics.py` (`InProcessMetrics` on `app.state.metrics`) and are scraped at `GET /metrics`. Generation success writes a durable `audit_events` row through `AuditRepository`. See [ADR 0007](adr/0007-metrics-tracing-audit.md).
...
The first AWS slice is a Terraform root module at `infra/terraform`. It creates a two-AZ VPC, one NAT gateway, a Fargate cluster, and a public HTTP load balancer aimed at port 8000. The image pipeline adds an immutable ECR repository, an ECS service on that cluster and target group, task and deploy roles, one log group, and `.github/workflows/deploy.yml`, which pushes an image tagged with the commit SHA on every push to `main` and deploys only on a manual run, authenticating to AWS through OIDC. It does not create a database, cache, secrets store, or CloudWatch dashboards and alarms, so a deployed task serves `GET /health` but not `POST /v1/responses` until those exist. CI validates the module and does not apply it. See [ADR 0009](adr/0009-aws-terraform-foundation.md) and [ADR 0010](adr/0010-ecr-image-pipeline-and-ecs-service.md).
