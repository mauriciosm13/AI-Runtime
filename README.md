...
The project has a minimal FastAPI bootstrap with typed environment configuration and an operational liveness endpoint at `GET /health`. Provider-neutral generation contracts live in `domain/`, `ports/` defines external capability interfaces (`ModelProvider`, `OrganizationRepository`, `ApiKeyRepository`, `ApiKeyHasher`, `UsageRepository`, `CostEstimator`, `ResponseCache`), and `application/` contains use cases (`CreateResponse`, `ModelRouter`, `CreateOrganization`, `GetOrganization`, `CreateApiKey`, `RevokeApiKey`, `ListApiKeysForOrganization`, `AuthenticateApiKey`). Concrete provider adapters are `OpenAIModelProvider` (`providers/openai/`, OpenAI Chat Completions over `httpx`), `AnthropicModelProvider` (`providers/anthropic/`, Anthropic Messages over `httpx`), and `GeminiModelProvider` (`providers/gemini/`, Gemini generateContent over `httpx`). `CreateResponse` resolves the requested model through `ModelRouter` against `DEFAULT_MODEL_CATALOG` (`gpt-4o`, `gpt-4o-mini` → `openai`; `claude-3-5-sonnet-20241022` → `anthropic`; `gemini-2.5-flash` → `gemini`) before calling a provider. `POST /v1/responses` requires `Authorization: Bearer airt_...`, runs `AuthenticateApiKey` (prefix lookup + argon2id verify, suspended-org rejection), then `CreateResponse` through HTTP with Pydantic schemas, dependency injection, and a shared `httpx.AsyncClient` managed by the application lifespan. Clients may set `stream: true` to receive Server-Sent Events (`response.delta`, `response.completed`, `response.error`); omitting `stream` or setting `false` keeps the JSON contract. `Idempotency-Key` is rejected on streaming requests. Clients may set `cache: true` on non-streaming requests to replay an identical payload from Redis (`cached: true` on hit); `cache` plus `stream` is rejected. Usage is recorded after a completed stream, not after a mid-stream failure. After a successful provider response, `CreateResponse` persists a `UsageRecord` (tokens + estimated USD cost via `StaticCostEstimator`) keyed by `request_id`, with `provider` taken from the resolved route. `GET /health` and `GET /metrics` stay unauthenticated. `GET /metrics` exposes Prometheus text for HTTP and generation counters. Successful generations write an `audit_events` row without prompt or response content. Client-facing errors use a standardized provider-neutral envelope with stable error codes (`unauthorized`, `forbidden`, `unsupported_model`, …) and a per-request correlation identifier (`X-Request-ID`). Structured JSON request logs record request start and completion with the same identifier and never log Authorization headers or secrets. Persistence wiring uses SQLAlchemy 2 async with asyncpg: the application lifespan owns the engine and session factory, and FastAPI can inject request-scoped `AsyncSession` values. Organization tenancy is modeled in `domain/organization.py`, persisted through `SqlAlchemyOrganizationRepository` and the `organizations` table (Alembic revision `0002_organizations`). API keys are modeled in `domain/api_key.py`, hashed at rest with argon2id (`Argon2ApiKeyHasher`), persisted through `SqlAlchemyApiKeyRepository` and the `api_keys` table (Alembic revision `0003_api_keys`). Usage accounting is modeled in `domain/usage.py` and persisted through `SqlAlchemyUsageRepository` and the `usage_records` table (Alembic revision `0004_usage_records`). Create returns the plaintext `airt_...` secret once; only prefix + hash are stored. `POST /v1/responses` also accepts `prompt: {name, version?, variables}` instead of `messages` to render an organization-scoped, versioned prompt template (managed through `POST /v1/prompts` and `GET /v1/prompts/{name}`), plus an opt-in `context` budget with truncation; templates are stored as configuration and rendered content is never persisted. AWS network and an empty Fargate cluster are defined in `infra/terraform` ([ADR 0009](docs/adr/0009-aws-terraform-foundation.md)); CI validates the module and does not apply it. Operator HTTP routes for organizations/API keys, usage reporting/quotas, and persisted routing policies are not implemented yet.
...
src/
  ai_runtime/          # distributable application package
    ...
    providers/         # concrete model-provider adapters (OpenAI, Anthropic, and Gemini via httpx)
...
| `AI_RUNTIME_GEMINI_API_KEY` | `gemini_api_key` | `str` | `""` |
| `AI_RUNTIME_GEMINI_BASE_URL` | `gemini_base_url` | `str` | `https://generativelanguage.googleapis.com` |
...
`POST /v1/responses` enforces a platform default per-organization token-bucket rate limit and honors an optional `Idempotency-Key` header (replay on success, `409` while in progress). Clients may set `cache: true` to store a successful non-streaming response in Redis for a short TTL and replay it for identical requests in the same organization. When Redis is down, rate limiting, idempotency, and cache fail open.
